...
export default function MoreResult() {
  ...
  const existingIdeas = useMemo(
    () =>
      location.state?.existingIdeas ||
      JSON.parse(localStorage.getItem("latestMoreExistingIdeas") || "null") ||
      [],
    [location.state]
  );
  ...
  useEffect(() => {
    const stored =
      ...
      readIdeasPayload(MORE_RESULTS_KEY);
  ...
  const handleSaveFullPlan = async () => {
    ...
    try {
      ...
      if (results.every((item) => item.success)) {
        toast.success("Fresh topic ideas saved to Content Ideas!");
      }
    } catch (e) {
  ...
  const filteredIdeas = (generatedResults?.ideas || []).filter((idea) => {
  ...
  if (!generatedResults || !generatedResults.ideas || generatedResults.ideas.length === 0) {
    return (
      <WorkspaceShell
        ...
        section="Results"
        title="No fresh content ideas found"
        description="We couldn't recover the latest extra idea batch."
        backTo="/ResultPage"
      ...
      >
        <div className="flex min-h-[60vh] flex-col items-center justify-center text-center">
          ...
          <h2 className="text-3xl font-black" style={{ color: "var(--app-text)" }}>Need another batch?</h2>
          <p className="mt-4 max-w-md" style={{ color: "var(--muted-text)" }}>
            Go back to your strategy page and generate a fresh set of additional ideas.
          </p>
  ...
  return (
    <WorkspaceShell
      badge="Fresh AI Batch"
      section="Results"
      title="Fresh content ideas for you"
      description="We've generated a new batch of ideas that are distinct from your previous ones. Explore new angles and hooks."
      backTo="/ResultPage"
      backLabel="Main Strategy"
      actions={
        <>
    ...
    >
      ...
      <div className="mb-12 text-center">
        ...
        <p className="mx-auto mt-4 max-w-2xl text-lg" style={{ color: "var(--muted-text)" }}>
          A secondary round of ideas with fresh hooks and stronger angles to complement your strategy.
        </p>
      </div>
      ...
      <div className="mb-8 flex items-center gap-3">
        ...
        <div>
          <p className="text-[10px] font-bold uppercase tracking-widest text-orange-500">Fresh AI Batch</p>
          <h3 className="text-lg font-black" style={{ color: "var(--app-text)" }}>New Topic Ideas</h3>
        </div>
      </div>
            ...
            <div className="mb-2 flex items-start gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-xs leading-relaxed" style={{ color: "var(--muted-text)" }}>
                  {idea.description || `Strategic approach to ${idea.title} focused on maximizing your brand's presence.`}
                </p>
              </div>
            </div>
            ...
            <div className="mt-4 flex-1 space-y-2">
              ...
              <div className="rounded-2xl p-4" style={{ background: "rgba(248,113,113,0.06)", border: "1px solid rgba(248,113,113,0.15)" }}>
                ...
                <p className="text-sm leading-relaxed" style={{ color: "var(--app-text)" }}>
                  {idea.painPoint || "Solving a specific audience challenge."}
                </p>
              </div>
            </div>
              ...
              onClick={() =>
                navigate("/WeeklyPlanPage", {
                  state: {
                    selectedIdea: idea,
                    onboardingData,
                    sourceResults: [...existingIdeas, ...(generatedResults?.ideas || [])],
                    sourcePage: "more-results",
                    fromDashboard: false,
                  },
                })
              }
      ...
      <div className="mt-16 flex flex-wrap justify-center gap-4">
        ...
        >
          <ArrowLeft size={18} /> Back to Main Strategy
        </button>
        <button
          onClick={() =>
            navigate("/MoreLoading", { state: { onboardingData, existingIdeas: [...existingIdeas, ...generatedResults.ideas] } })
          }
          ...
          style={{ boxShadow: "0 8px 30px rgba(249,115,22,0.35)" }}
        >
          <Plus size={18} /> Generate Even More
        </button>
      </div>
    </WorkspaceShell>
  );
