...
function MoreLoading() {
  ...
  const existingIdeas =
    location.state?.existingIdeas ||
    readIdeasPayload(GENERATED_RESULTS_KEY)?.ideas ||
    [];

  const loadingSteps = [
    {
      title: "Analyzing your current content ideas...",
      description:
        "We are checking which content styles, hooks, and formats perform best for your audience.",
      icon: Sparkles,
    },
    {
      title: "Finding fresh new content opportunities...",
      description:
        "We are discovering new trends, topics, and content angles for better reach.",
      icon: Lightbulb,
    },
    {
      title: "Generating more high-quality ideas...",
      description:
        "We are creating new content ideas with stronger hooks and better engagement potential.",
      icon: Wand2,
    },
  ];
 ...
 useEffect(() => {
  ...
  const runMoreGeneration = async () => {
    if (!onboardingData) {
      toast.error("Missing onboarding data. Please generate your main strategy again.");
      navigate("/ResultPage");
      return;
    }
    ...
    const generationPromise = fetch("http://localhost:5000/api/auth/generate-results", {
      ...
      body: JSON.stringify({
        ...
        options: {
          ideaCount: 10,
          excludeTitles: existingIdeas.map((idea) => idea.title).filter(Boolean),
          currentIdeas: existingIdeas.slice(0, 10),
        },
      }),
    })
      .then(async (response) => {
        ...
        if (!response.ok) {
          throw new Error(data?.message || "Failed to generate more results");
        }

        if (!data?.results || !Array.isArray(data.results.ideas)) {
          throw new Error("Invalid results payload received from server");
        }

        return data.results;
      })
      .catch((error) => {
        console.error("More loading generation error:", error);
        return {
          ideas: [],
          error: error.message || "Failed to generate more results",
        };
      });
    ...
    if (
      !generatedResults ||
      !Array.isArray(generatedResults.ideas) ||
      generatedResults.ideas.length === 0
    ) {
      toast.error(generatedResults?.error || "Generate More failed. Please try again.");
      navigate("/ResultPage");
      return;
    }
    ...
    // 🔥 REMOVE DUPLICATES
    const uniqueIdeas = mergedIdeas.filter(
      (idea, index, self) =>
        index === self.findIndex((i) => i.title === idea.title)
    );

    // ✅ SAVE
    writeIdeasPayload(GENERATED_RESULTS_KEY, { ideas: uniqueIdeas });
    writeIdeasPayload(MORE_RESULTS_KEY, {
      ideas: generatedResults?.ideas || [],
    });
    localStorage.setItem(
      "latestMoreExistingIdeas",
    ...
    navigate("/MoreResult", {
      state: {
        onboardingData,
        generatedResults: { ideas: generatedResults?.ideas || [] },
        existingIdeas,
      },
    });
  }; // ✅ IMPORTANT: function closed properly

  runMoreGeneration(); // ✅ outside function
...
}, [navigate, onboardingData, existingIdeas]); 
      ...
      >
        ...
        <div className="relative flex flex-col items-center text-center">
          ...
          <div className="relative mb-8 animate-[loading_float_4s_ease-in-out_infinite]">
            ...
            <div className="relative flex h-32 w-32 items-center justify-center">
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="flex h-12 w-12 items-center justify-center rounded-full bg-orange-500 text-white shadow-[0_0_20px_rgba(244,124,53,0.6)]">
          ...
          <div className="mt-10 flex flex-wrap items-center justify-center gap-3">
            {[
              { label: "New Hooks", color: "#f97316", bg: "rgba(249,115,22,0.06)" },
              { label: "Better Angles", color: "#3b82f6", bg: "rgba(59,130,246,0.06)" },
              { label: "Higher Reach", color: "#22c55e", bg: "rgba(34,197,94,0.06)" }
            ].map((item) => (
              <div
                key={item.label}
